...
The exact method signatures are:
...
Map preserves source order. Flat-map preserves source and nested-stream order. Empty streams
remain empty, and source or nested errors remain observable under the existing contract.
...
Run the feature [quickstart](../specs/002-go-1-27-upgrade/quickstart.md), including Go 1.27.x
and stable build/test/vet gates, Linux race validation, typed stream regressions, targeted
Go 1.27 modernizers, and both examples before tagging v0.13.0.
